...
describe('UserRepository', () => {
  ...
  describe('hashPassword', () => {
    it('calls bcrypt.hash to generate a hash', async () => {
      const returnedHash = 'testHash';
      const [password, salt] = ['testPassword', 'testSalt'];
      const spy = jest.spyOn(bcrypt, 'hash');
      spy.mockImplementation(() => Promise.resolve(returnedHash)); // spy.mockResolvedValue(returnedHash);
      expect(bcrypt.hash).not.toHaveBeenCalled();
      const result = await userRepository.hashPassword(password, salt);
      expect(bcrypt.hash).toHaveBeenCalledWith(password, salt);
      expect(result).toEqual(returnedHash);
    });
  });
});
